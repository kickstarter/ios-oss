...
class HTMLParser {
    ...
    private func parse(_ children: Elements?) -> [ViewElement] {
        ...
        children?.forEach({ (element) in
            let viewElementType = ViewElementType.initialize(element: element)
                   ...
                   let imageElements = try? element.getElementsByTag("img") {
                    for imageElement in imageElements {
                        if let sourceUrl = imageElement.dataset()["src"] {
                            viewElements.append(EmbeddedLinkViewElement(href: href.getValue(), src: sourceUrl, caption: caption))
                        }
                    }
                }
                return
...
//                viewElements.append(UnknownViewElement(text: "tag: \(element.tag().getName()) \(element.dataset().description)"))
    ...
    private func parseTextElement(element: Element, tags: [String] = [], textComponents: [TextComponent] = []) -> [TextComponent] {
        ...
        for node in element.getChildNodes() {
            if let textNode = node as? TextNode {
                ...
                components.append(TextComponent(text: textNode.text(),
                                                link: href,
                                                styles: elementTags.map { TextStyleType.initalize(tag: $0) }))
            } else if let nodeElement = node as? Element {
                components.append(contentsOf: parseTextElement(element: nodeElement,
                                                               tags: elementTags,
