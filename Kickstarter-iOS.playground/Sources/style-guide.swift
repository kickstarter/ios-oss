 ...
 */
public func colorPalette(colors colors: [(color: UIColor, name: String)],
  size: CGSize = CGSize(width: 800.0, height: 400.0),
  columnsCount: Int = 6)
  -> UIView {

  let palette = UIView(frame: CGRect(origin: .zero, size: size))
  palette.backgroundColor = .clearColor()
  ...
  let tiles = colors.enumerate().map { idx, data -> UIView in
    ...
    label.textColor = data.color.white < 0.5 ? .whiteColor() : .blackColor()
