...
final class FormatTests: TestCase {
  ...
  func testAttributedCurrency() {
    withEnvironment(locale: Locale(identifier: "en")) {
      withEnvironment(countryCode: "US") {
        ...
        XCTAssertEqual(Format.attributedCurrency(1_000, country: .gr)?.string, "€1,000.00")
        XCTAssertEqual(Format.attributedCurrency(1_000, country: .si)?.string, "€1,000.00")
        XCTAssertEqual(Format.attributedCurrency(1_000, country: .pl)?.string, "zł 1,000.00")
  ...
  func testAttributedCurrencyFromStringAmount() {
    withEnvironment(locale: Locale(identifier: "en")) {
      withEnvironment(countryCode: "US") {
        XCTAssertEqual(Format.attributedCurrency(amountString: "1,000.00", country: .us)?.string, "$1,000.00")
        XCTAssertEqual(
          Format.attributedCurrency(amountString: "1,000.00", country: .ca)?.string,
          " CA$ 1,000.00"
        )
        XCTAssertEqual(Format.attributedCurrency(amountString: "1,000.00", country: .gb)?.string, "£1,000.00")
        XCTAssertEqual(
          Format.attributedCurrency(amountString: "1,000.00", country: .dk)?.string,
          " DKK 1,000.00"
        )
        XCTAssertEqual(Format.attributedCurrency(amountString: "1,000.00", country: .de)?.string, "€1,000.00")
        XCTAssertEqual(Format.attributedCurrency(amountString: "1,000.00", country: .jp)?.string, "¥1,000.00")
        XCTAssertEqual(Format.attributedCurrency(amountString: "1,000.00", country: .gr)?.string, "€1,000.00")
        XCTAssertEqual(Format.attributedCurrency(amountString: "1,000.00", country: .si)?.string, "€1,000.00")
        XCTAssertEqual(
          Format.attributedCurrency(amountString: "1,000.00", country: .pl)?.string,
          "zł1,000.00"
        )

        // Test European notation
        XCTAssertEqual(Format.attributedCurrency(amountString: "1.000,00", country: .gb)?.string, "£1.000,00")

        // Test omitCurrencyCode for US
        XCTAssertEqual(
          Format.attributedCurrency(amountString: "1,000.00", country: .us, omitCurrencyCode: true)?.string,
          "$1,000.00"
        )
        XCTAssertEqual(
          Format.attributedCurrency(amountString: "1,000.00", country: .us, omitCurrencyCode: false)?.string,
          " US$ 1,000.00"
        )

        // Test omitCurrencyCode for a non US country
        XCTAssertEqual(
          Format.attributedCurrency(amountString: "1.000,00", country: .gr, omitCurrencyCode: true)?.string,
          "€1.000,00"
        )
        XCTAssertEqual(
          Format.attributedCurrency(amountString: "1.000,00", country: .gr, omitCurrencyCode: false)?.string,
          "€1.000,00"
        )
      }
    }
  ...
  func testParseAmountComponents() {
    var result = Format.parseAmountComponents(from: "499.75")
    XCTAssertNotNil(result)
    XCTAssertEqual(result?.integerPart, "499")
    XCTAssertEqual(result?.decimalSeparator, ".")
    XCTAssertEqual(result?.decimalPart, "75")

    result = Format.parseAmountComponents(from: "499,75")
    XCTAssertNotNil(result)
    XCTAssertEqual(result?.integerPart, "499")
    XCTAssertEqual(result?.decimalSeparator, ",")
    XCTAssertEqual(result?.decimalPart, "75")

    result = Format.parseAmountComponents(from: "1000.34")
    XCTAssertNotNil(result)
    XCTAssertEqual(result?.integerPart, "1000")
    XCTAssertEqual(result?.decimalSeparator, ".")
    XCTAssertEqual(result?.decimalPart, "34")
    ...
    result = Format.parseAmountComponents(from: "2,499.00")
    XCTAssertNotNil(result)
    XCTAssertEqual(result?.integerPart, "2,499")
    XCTAssertEqual(result?.decimalSeparator, ".")
    ...
    result = Format.parseAmountComponents(from: "1000")
    XCTAssertNil(result)
  }
  ...
  func testCurrency() {
    withEnvironment(locale: Locale(identifier: "en")) {
      withEnvironment(countryCode: "US") {
        ...
        XCTAssertEqual(Format.currency(1_000, country: .gr), "€1,000")
        XCTAssertEqual(Format.currency(1_000, country: .si), "€1,000")
        XCTAssertEqual(Format.currency(1_000, country: .pl), "zł 1,000")
    ...
    withEnvironment(locale: Locale(identifier: "es")) {
      withEnvironment(countryCode: "US") {
        ...
        XCTAssertEqual(Format.currency(1_000, country: .gr), "1000 €")
        XCTAssertEqual(Format.currency(1_000, country: .si), "1000 €")
        XCTAssertEqual(Format.currency(1_000, country: .pl), "1000 zł")
      }
    ...
    withEnvironment(locale: Locale(identifier: "fr")) {
      withEnvironment(countryCode: "US") {
        ...
        XCTAssertEqual(Format.currency(1_000, country: .gr), "1 000 €")
        XCTAssertEqual(Format.currency(1_000, country: .si), "1 000 €")
        XCTAssertEqual(Format.currency(1_000, country: .pl), "1 000 zł")
      }
    ...
    withEnvironment(locale: Locale(identifier: "jp")) {
      withEnvironment(countryCode: "DK") {
        ...
        XCTAssertEqual(Format.currency(1_000, country: .gr), "€ 1000")
        XCTAssertEqual(Format.currency(1_000, country: .si), "€ 1000")
        XCTAssertEqual(Format.currency(1_000, country: .pl), "zł 1000")
      }
    }
