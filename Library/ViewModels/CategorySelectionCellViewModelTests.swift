...
final class CategorySelectionCellViewModelTests: TestCase {
  ...
  private let loadSubcategoriesStyle = TestObserver<PillCellStyle, Never>()
  ...
  override func setUp() {
    ...
    self.vm.outputs.loadSubCategories.map { $0.map(first) }.observe(self.loadSubcategories.observer)
    self.vm.outputs.loadSubCategories.map { $0.map(second).first }.skipNil()
      .observe(self.loadSubcategoriesStyle.observer)
  }
  ...
  func testLoadSubcategories() {
    ...
    self.loadSubcategoriesStyle.assertDidNotEmitValue()

    self.vm.inputs.configure(with: category)

    self.loadSubcategories.assertValues([["All Art Projects", "Illustration"]])
    self.loadSubcategoriesStyle.assertValues([.grey])
  }
}
