...
public final class FindFriendsFriendFollowCellViewModel: FindFriendsFriendFollowCellViewModelType,
  FindFriendsFriendFollowCellViewModelInputs, FindFriendsFriendFollowCellViewModelOutputs {
    ...
    let followedText = localizedString(
      key: "Followed",
      defaultValue: "Followed",
      count: nil,
      substitutions: [:])

    let notFollowedText = localizedString(
      key: "Not_followed",
      defaultValue: "Not followed",
      count: nil,
      substitutions: [:])

    self.cellAccessibilityLabel = Signal.combineLatest(self.name, self.projectsBackedText, isFollowed)
      .map { "\($0.0), \($0.1). " + ($0.2 ? followedText : notFollowedText) }
