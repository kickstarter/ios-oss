...
public final class ReferralChartView: UIView {
  ...
  public override func draw(_ rect: CGRect) {
    ...
    let internalPercentageAngle = CGFloat(-Double.pi / 2) + self.internalPercentage * CGFloat(2.0 * Double.pi)
    let internalAndExternalPercentage = min(self.internalPercentage + self.externalPercentage, 1.0)
    let internalAndExternalPercentageAngle = CGFloat(-Double.pi / 2) + internalAndExternalPercentage *
      CGFloat(2.0 * Double.pi)
    ...
    context.addArc(center: .init(x: self.bounds.width/2, y: self.bounds.height/2),
                   radius: self.bounds.width/2,
                   startAngle: CGFloat(-Double.pi / 2.0) + self.internalPercentage * CGFloat(2.0 * Double.pi),
                   endAngle: CGFloat(-Double.pi / 2.0) + internalAndExternalPercentage * CGFloat(2.0 * Double.pi),
                   clockwise: false)
    context.closePath()
    context.fillPath()
    ...
    context.addArc(center: .init(x: self.bounds.width/2, y: self.bounds.height/2),
                   ...
                   startAngle: CGFloat(-Double.pi / 2.0) + internalAndExternalPercentage * CGFloat(2.0 * Double.pi),
