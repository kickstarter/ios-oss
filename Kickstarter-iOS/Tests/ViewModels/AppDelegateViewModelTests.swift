...
final class AppDelegateViewModelTests: TestCase {
  ...
  override func setUp() {
    ...
    self.vm.outputs.goToLiveStream.map { $0.0 }.map(first).observe(self.goToLiveStreamProject.observer)
    self.vm.outputs.goToLiveStream.map { $0.0 }.map(second).observe(self.goToLiveStreamLiveStream.observer)
    self.vm.outputs.goToLiveStream.map { $0.0 }.map(third).observe(self.goToLiveStreamLiveStreamEvent.observer)
    self.vm.outputs.goToLiveStream.map { $0.1 }.observe(self.goToLiveStreamRefTag.observer)
