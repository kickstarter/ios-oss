...
final class LiveStreamChatDataSourceTests: TestCase {
  ...
  func testDataSource() {
    let chatMessage1 = .template
      |> LiveStreamChatMessage.lens.id .~ "1"
    let chatMessage2 = .template
      |> LiveStreamChatMessage.lens.id .~ "2"
    let chatMessage3 = .template
      |> LiveStreamChatMessage.lens.id .~ "3"
    let chatMessage4 = .template
      |> LiveStreamChatMessage.lens.id .~ "4"
    let chatMessage5 = .template
      |> LiveStreamChatMessage.lens.id .~ "5"
