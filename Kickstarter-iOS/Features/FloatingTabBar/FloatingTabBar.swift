...
final class FloatingTabBar: UITabBar {
  ...
  /// The total width of the floating tab bar.
  private func tabBarWidth(for items: CGFloat) -> CGFloat {
    let horizontalInset = self.metrics.scaledValue(for: Constants.tabBarHorizontalInset)
    let interItemSpacing = self.metrics.scaledValue(for: Spacing.unit_02)

    let iconWidthTotal = items * self.itemSize
    let spacingTotal = max(items - 1, 0) * interItemSpacing

    return (horizontalInset * 2) + iconWidthTotal + spacingTotal
  }
