...
public protocol AppDelegateViewModelOutputs {
  ...
  var goToLiveStream: Signal<(ProjectLiveStreamData, RefTag?), NoError> { get }
...
AppDelegateViewModelOutputs {
  ...
  public let goToLiveStream: Signal<(ProjectLiveStreamData, RefTag?), NoError>
...
private func liveStreamData(fromNavigation nav: Navigation)
  -> SignalProducer<(ProjectLiveStreamData, RefTag?), NoError> {
    ...
    return SignalProducer.zip(
      ...
      .map { project, liveStreamEvent -> (ProjectLiveStreamData, RefTag?)? in
        guard let liveStream = project.liveStreams.first(where: { $0.id == eventId }) else { return nil }
        return ((project: project, liveStream: liveStream, event: liveStreamEvent), refTag)
      }
      .skipNil()
}
