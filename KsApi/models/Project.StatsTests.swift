...
final class ProjectStatsTests: XCTestCase {
  ...
  func testGoalCurrentCurrency() {
    ...
    // 1000 * 0.2999 = 299.9
    XCTAssertEqual(299, projectProjectUserCurrencyRate.stats.goalCurrentCurrency, "Converted goal amount rounds down")
  }
  ...
  func testCurrentCountry() {
    ...
    XCTAssertEqual(Project.Country.us, project.stats.currentCountry, "Current country returns country when currency code corresponds to known country")

    let projectUnknownCurrency = .template
      |> Project.lens.stats.currentCurrency .~ "ZZZ"

    XCTAssertNil(projectUnknownCurrency.stats.currentCountry, "Current country returns nil when currency code does not correspond to known country")
  ...
  func testOmitUSCurrencyCode() {
    ...
    XCTAssertTrue(project.stats.omitUSCurrencyCode, "Omits US currency code for users with a preferred currency of USD")

    let projectNilCurrentCurrency = project
      |> Project.lens.stats.currentCurrency .~ nil

    XCTAssertTrue(projectNilCurrentCurrency.stats.omitUSCurrencyCode, "Omits US currency code for users without a preferred currency (which then defaults to USD)")

    let projectNonUSCurrentCurrency = project
      |> Project.lens.stats.currentCurrency .~ "SEK"

    XCTAssertFalse(projectNonUSCurrentCurrency.stats.omitUSCurrencyCode, "Does not omit US currency code for users whose preferred currency is not USD")
  }
  ...
  func testNeedsConversion() {
    ...
    XCTAssertTrue(project.stats.needsConversion, "Needs conversion is true when project currency is different from users preferred currency")

    let projectUnknownPreferredCurrency = .template
      |> Project.lens.stats.currentCurrency .~ nil
      |> Project.lens.stats.currency .~ Project.Country.au.currencyCode

    XCTAssertTrue(projectUnknownPreferredCurrency.stats.needsConversion, "Needs conversion is true when project currency is not USD, and the user has no preferred currency")

    let projectSameCurrency = .template
      |> Project.lens.stats.currentCurrency .~ Project.Country.mx.currencyCode
      |> Project.lens.stats.currency .~ Project.Country.mx.currencyCode

    XCTAssertFalse(projectSameCurrency.stats.needsConversion, "Needs conversion is false when project currency is the same as users preferred currency")
  }
}
